      ...
      <div className="bg-white rounded-3xl border border-slate-200 p-5 sm:p-6 shadow-sm space-y-5">
        ...
        <div className="flex flex-wrap items-center gap-2 border-b border-slate-100 pb-2">
          ...
          >
            ...
            {rules.length > 0 && (
              <span className={`text-[10px] px-1.5 py-0.2 rounded-full font-extrabold ${activeSubTab === 'rules' ? 'bg-white/20 text-white' : 'bg-slate-300 text-slate-800'}`}>
                {activeRulesCount}
              </span>
            )}
          </button>
          ...
          >
            ...
            {goals.length > 0 && (
              <span className={`text-[10px] px-1.5 py-0.2 rounded-full font-extrabold ${activeSubTab === 'goals' ? 'bg-white/20 text-white' : 'bg-slate-300 text-slate-800'}`}>
                  ...
                  <div
                    ...
                    className={`p-3.5 rounded-2xl border flex items-center justify-between text-xs transition ${
                      rule.active
                        ...
                        : 'bg-slate-100/60 border-slate-200 text-slate-400 line-through opacity-70'
                    }`}
                  >
            ...
            <div className="space-y-2.5">
              {PRO_RULE_PRESETS.map((preset, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className="p-3 rounded-2xl border border-slate-200 bg-slate-50/70 hover:bg-slate-100/80 transition space-y-2"
                  >
                    <div className="flex items-center justify-between">
          ...
          <div className="bg-white rounded-3xl border border-slate-200 p-5 shadow-sm space-y-4">
            ...
            <div className="space-y-2.5">
              {PRO_GOAL_PRESETS.map((preset, idx) => (
                <div key={idx} className="p-3 rounded-2xl border border-slate-200 bg-slate-50/80 space-y-2">
                  <div className="flex items-center justify-between">
